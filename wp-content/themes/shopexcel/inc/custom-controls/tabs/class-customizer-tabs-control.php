...
class Customizer_Tabs_Control extends WP_Customize_Control {
	...
	public function enqueue_tabs_script() {
		...
		wp_enqueue_script( 'tabs-control-script', get_template_directory_uri() . '/inc/custom-controls/tabs' . $build . '/script' . $suffix . '.js', array( 'jquery' ), SHOPEXCEL_THEME_VERSION, true );
		wp_enqueue_style( 'tabs-control-style', get_template_directory_uri() . '/inc/custom-controls/tabs' . $build . '/tabs' . $suffix . '.css', null, SHOPEXCEL_THEME_VERSION );
	}
	...
	public function partials_helper_script_enqueue() {
		...
		$build  = ( defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ) ? '/build' : '';
		$suffix = ( defined( 'SCRIPT_DEBUG' ) && SCRIPT_DEBUG ) ? '' : '.min';
		wp_enqueue_script( 'tabs-addon-script', get_template_directory_uri() . '/inc/custom-controls/tabs' . $build . '/customizer-addon-script' . $suffix . '.js', array( 'jquery' ), SHOPEXCEL_THEME_VERSION, true );
	}
