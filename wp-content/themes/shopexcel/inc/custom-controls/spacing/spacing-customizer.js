( function( $, api ) {
	...
	api.controlConstructor['spacing'] = api.Control.extend( {
		ready: function() {
			var control = this;
			$( '.shopexcel-number-control', control.container ).on( 'change keyup',
				function() {
	...
	api.controlConstructor['shopexcel-spacing-slider'] = api.Control.extend( {
		ready: function() {
			var control = this;
	...
	api.controlConstructor['shopexcel-spacing'] = api.Control.extend( {
		ready: function() {
			var control = this,
				controlClass = '.customize-control-shopexcel-spacing',
				footerActions = jQuery( '#customize-footer-actions' );

			// Figure out which device icon to make active on load
			jQuery( controlClass + ' .shopexcel-spacing-control-section' ).each( function() {
...
jQuery( document ).ready( function($) {
	$( '.shopexcel-link-spacing' ).on( 'click', function(e) {
		e.preventDefault();
		...
		_this.parent( '.shopexcel-spacing-section' ).prevAll().slice(0,4).find( 'input' ).addClass( 'linked-values' ).attr( 'data-element', element );
	...
	$( '.shopexcel-unlink-spacing' ).on( 'click', function(e) {
		e.preventDefault();
		...
		_this.parent( '.shopexcel-spacing-section' ).prevAll().slice(0,4).find( 'input' ).removeClass( 'linked-values' ).attr( 'data-element', '' );
	...
	$( '.shopexcel-spacing-section' ).on( 'input', '.linked-values', function() {
		var _this = $( this ),
