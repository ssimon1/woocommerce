( function( api ) {
	...
	api.controlConstructor['shopexcel-typography'] = api.Control.extend( {
		ready: function() {
			var control = this;

			control.container.on( 'change', '.shopexcel-font-family select',
				function() {
					...
					setTimeout( function() {
						// Send our request to the shopexcel_get_all_google_fonts_ajax function
						var response = jQuery.getJSON({
							type: 'POST',
							url: ajaxurl,
							data: {
								action: 'shopexcel_get_all_google_fonts_ajax',
								shopexcel_customize_nonce: shopexcel_customize.nonce
							},
							async: false,
							dataType: 'json',
						});
						...
						if ( id in fonts ) {
							...
							jQuery( '.shopexcel-font-family select' ).not( _this ).each( function( key, select ) {
								var parent = jQuery( this ).closest( '.shopexcel-font-family' );

								if ( _value == jQuery( select ).val() && _this.data( 'category' ) !== jQuery( select ).data( 'category' ) ) {
									if ( ! got_variants ) {
										updated_variants = jQuery( parent.next( '.shopexcel-font-variant' ).find( 'select' ) ).val();
										got_variants = true;
									}
								}
							} );
							...
							_this.closest( '.shopexcel-font-family' ).next( 'div' ).show();
							...
							_this.closest( '.shopexcel-font-family' ).next( 'div' ).hide();
			...
			control.container.on( 'change', '.shopexcel-font-variant select',
				function() {
					var _this = jQuery( this );
					...
					jQuery( '.shopexcel-font-variant select' ).each( function( key, value ) {
						var this_control = jQuery( this ).closest( 'li' ).attr( 'id' ).replace( 'customize-control-', '' );
						var parent = jQuery( this ).closest( '.shopexcel-font-variant' );
			...
			control.container.on( 'change', '.shopexcel-font-category input',
				function() {
					control.settings['category'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.shopexcel-font-weight select',
				function() {
					control.settings['weight'].set( jQuery( this ).val() );
				}
			);

			control.container.on( 'change', '.shopexcel-font-transform select',
				function() {
...
jQuery( document ).ready( function( $ ) {
	...
	$( '.shopexcel-font-family select' ).select2();

	$( '.shopexcel-font-variant' ).each( function( key, value ) {
		var _this = $( this );
	...
	$( ".shopexcel-font-family" ).each( function( key, value ) {
		var _this = $( this );
		if ( $.inArray( _this.find( 'select' ).val(), typography_defaults ) !== -1 ) {
			_this.next( '.shopexcel-font-variant' ).hide();
		}
	} );
