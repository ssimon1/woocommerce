wp.customize.controlConstructor['shopexcel-range-slider'] = wp.customize.Control.extend({
	...
	ready: function() {
		...
		var control = this,
		    value,
			controlClass = '.customize-control-shopexcel-range-slider',
			footerActions = jQuery( '#customize-footer-actions' );
		
		// Set up the sliders
		jQuery( '.shopexcel-slider' ).each( function() {
		...
		jQuery( controlClass + ' .shopexcel_range_value input[type=number]' ).on( 'input', function() {
			value = jQuery( this ).attr( 'value' );
			if ( '' == value ) {
				value = -1;
			}
			jQuery( this ).closest( 'label' ).find( '.shopexcel-slider' ).slider( 'value', parseFloat(value)).change();
		});

		// Handle the reset button
		jQuery( controlClass + ' .shopexcel-reset' ).on( 'click', function() {
			var icon = jQuery( this ),
				visible_area = icon.closest( '.shopexcel-range-title-area' ).next( '.shopexcel-range-slider-areas' ).children( 'label:visible' ),
				input = visible_area.find( 'input[type=number]' ),
				slider_value = visible_area.find( '.shopexcel-slider' ),
				visual_value = visible_area.find( '.shopexcel_range_value' ),
		...
		jQuery( controlClass + ' .shopexcel-range-slider-control' ).each( function() {
			var _this = jQuery( this );
			_this.find( '.shopexcel-device-controls' ).children( 'span:first-child' ).addClass( 'selected' );
			_this.find( '.range-option-area:first-child' ).show();
		});
		
		// Do stuff when device icons are clicked
		jQuery( controlClass + ' .shopexcel-device-controls > span' ).on( 'click', function( event ) {
			var device = jQuery( this ).data( 'option' );
			
			jQuery( controlClass + ' .shopexcel-device-controls span' ).each( function() {
				var _this = jQuery( this );
			...
			jQuery( controlClass + ' .shopexcel-range-slider-areas label' ).each( function() {
				var _this = jQuery( this );
				if ( device == _this.attr( 'data-option' ) ) {
					_this.show();
					_this.siblings().hide();
