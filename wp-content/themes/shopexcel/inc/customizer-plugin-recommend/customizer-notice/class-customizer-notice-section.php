<?php
/**
 * Shopexcel Customizer Notification Section Class.
 *
 * @package Shopexcel
 */

/**
 * Shopexcel_Customizer_Notice_Section class
 */
class Shopexcel_Customizer_Notice_Section extends WP_Customize_Section {
	...
	public function call_plugin_api( $slug ) {
		...
		$call_api = get_transient( 'shopexcel_cust_notice_plugin_info_' . $slug );
		if ( false === $call_api ) {
			...
			set_transient( 'shopexcel_cust_notice_plugin_info_' . $slug, $call_api, 30 * MINUTE_IN_SECONDS );
		}
	...
	public function json() {
		$json = parent::json();
		global $shopexcel_recommended_plugins;
		...
		$show_recommended_plugins = get_option( 'shopexcel_show_recommended_plugins' );

		foreach ( $shopexcel_recommended_plugins as $slug => $plugin_opt ) {
			...
			$shopexcel_recommended_plugin['url']    = $this->create_action_link( $active['needs'], $slug );
			if ( $active['needs'] !== 'install' && $active['status'] ) {
				$shopexcel_recommended_plugin['class'] = 'active';
			} else {
				$shopexcel_recommended_plugin['class'] = '';
			}

			switch ( $active['needs'] ) {
				case 'install':
					$shopexcel_recommended_plugin['button_class'] = 'install-now button';
					$shopexcel_recommended_plugin['button_label'] = $install_button_label;
					break;
				case 'activate':
					$shopexcel_recommended_plugin['button_class'] = 'activate-now button button-primary';
					$shopexcel_recommended_plugin['button_label'] = $activate_button_label;
					break;
				case 'deactivate':
					$shopexcel_recommended_plugin['button_class'] = 'deactivate-now button';
					$shopexcel_recommended_plugin['button_label'] = $deactivate_button_label;
					break;
			}
            
			$info   = $this->call_plugin_api( $slug );
			$shopexcel_recommended_plugin['id'] = $slug;
			$shopexcel_recommended_plugin['plugin_slug'] = $slug;

			if ( ! empty( $plugin_opt['description'] ) ) {
				$shopexcel_recommended_plugin['description'] = $plugin_opt['description'];
			} else {
				$shopexcel_recommended_plugin['description'] = $info->short_description;
			}

			$shopexcel_recommended_plugin['title'] = $info->name;

			$customize_plugins[] = $shopexcel_recommended_plugin;
				...
				<div class="recomended-actions_container" id="plugin-filter">
					<# if( data.recommended_plugins.length > 0 ){ #>
						<# for (action in data.recommended_plugins) { #>
							<div class="shopexcel-recommeded-actions-container shopexcel-recommended-plugins" data-index="{{ data.recommended_plugins[action].index }}">
								<# if( !data.recommended_plugins[action].check ){ #>
									<div class="shopexcel-recommeded-actions">
