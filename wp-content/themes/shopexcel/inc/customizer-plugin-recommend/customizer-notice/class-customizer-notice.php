<?php
/**
 * Shopexcel Customizer Notification System.
 *
 * @package Shopexcel
 */
 ...
 */
class Shopexcel_Customizer_Notice {
	...
	/**
	 * The single instance of Shopexcel_Customizer_Notice
	 *
	 * @var Shopexcel_Customizer_Notice $instance The Shopexcel_Customizer_Notice instance.
	 */
	...
	/**
	 * The Main Shopexcel_Customizer_Notice instance.
	 *
	 * We make sure that only one instance of Shopexcel_Customizer_Notice exists in the memory at one time.
	 *
	 * @param array $config The configuration array.
	 */
	public static function init( $config ) {
		if ( ! isset( self::$instance ) && ! ( self::$instance instanceof Shopexcel_Customizer_Notice ) ) {
			self::$instance = new Shopexcel_Customizer_Notice;
	...
	public function setup_config() {
		...
		$shopexcel_recommended_plugins = array();
		
		if ( isset( $this->recommended_plugins ) ) {
			$shopexcel_recommended_plugins = $this->recommended_plugins;
		}
	...
	public function scripts_for_customizer() {
		...
		wp_enqueue_style( 'shopexcel-customizer-notice', get_template_directory_uri() . '/inc/customizer-plugin-recommend/customizer-notice/css/customizer-notice.css', null );
		...
		wp_enqueue_script( 'shopexcel-customizer-notice', get_template_directory_uri() . '/inc/customizer-plugin-recommend/customizer-notice/js/customizer-notice.js', array( 'jquery', 'customize-controls' ), SHOPEXCEL_THEME_VERSION, true );
		wp_localize_script(
			'shopexcel-customizer-notice', 
            'customizer_notice_data', 
            array(
				'ajaxurl'            => admin_url( 'admin-ajax.php' ),
				'template_directory' => get_template_directory_uri(),
				'base_path'          => admin_url(),
				'activating_string'  => __( 'Activating', 'shopexcel' ),
			)
		);
	}
	...
	public function customize_register_notice( $wp_customize ) {
		/**
		 * Include the Shopexcel_Customizer_Notice_Section class.
		 */
		...
		$wp_customize->register_section_type( 'Shopexcel_Customizer_Notice_Section' );

		$wp_customize->add_section(
			new Shopexcel_Customizer_Notice_Section(
				$wp_customize,
	...
	public function dismiss_recommended_plugins_callback() {
		...
		if ( ! empty( $action_id ) ) {
			...
			update_option( 'shopexcel_show_recommended_plugins', $show_recommended_plugins );
		}
